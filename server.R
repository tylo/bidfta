...
server <- function(input, output, session) {
    ...
    fileReaderData <- reactive({
        reactiveFileReader(5000, session, searches_loc,
                           get_lastN_searches, input$recent_searches_N)
    })
    ...
    output$pins_div <- renderUI({
        ...
        search_res$data %>%
            ...
            transmute(newcol = gen_search_output(Description,
                                                 link.item,
                                                 img_src,
                                                 date)) %>%
            unlist %>%
    ...
    output$search_df  <- DT::renderDataTable({
        ...
        search_res$data %>%
            left_join(auctions_df, by=c("Auction"="title")) %>%
            mutate(Photo = paste0('<a href="',
                                  link.item,
                                  '" target="_blank"><img src="',
                                  img_src,
                                  '" class="img-rounded" width="250"/></a>'),
                   Description = gen_search_output(Description,
                                                   link.item,
                                                   img_src,
                                                   date, pins = F)) %>%
            select(Photo, Description, MSRP, Auction)
